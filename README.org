...
lisp is built ON lisp, and when you work you will be working ON the language, there is no container,
you are outside a set of tools building upward.

In most languages you work within the constraints of the language
which is in turn in the operating system

Bewildering freedom, many operations we take for granted as together are
entirely orthogonal in CL. import is independent of use chdir is independent of
*default-directory-defaults* etc. Many granular steps are available to precise
the system, none of this is particularly verbose, butit does require a change of details to attend to.
If something is not working as expected, I check what actions I have assumed are bound.

the image is not quite an OS But it almost is, and it could be. When we work on
...
want, within a single user OS that we can design to acomplish a task or
